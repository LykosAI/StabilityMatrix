...
- Existing installs now refresh the bundled 7-Zip binary on startup whenever the app ships a newer version, instead of only when a package is freshly installed. Previously a stale binary could persist indefinitely for users who only ever updated existing packages
### Added
