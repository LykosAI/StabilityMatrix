...
Stability Matrix itself is distributed as a portable, self-contained desktop app, so separate installation of Python, Git, or packags is not usually required. In practice, the real hardware requirements come from the packages, models, and workflows a user wants to run.
