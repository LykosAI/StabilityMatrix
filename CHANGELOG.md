...
- The **Model Browser** now browses and imports **ComfyUI workflows** from CivitAI — pick the new **Workflows** model type, open one, and import it straight into your library; archives unpack automatically, and images with an embedded workflow become regular workflows
...
- **Custom node install assist**: after importing a workflow that needs custom nodes, Stability Matrix reads the node packs out of the workflow file, shows which you already have, and offers one-click installs for the rest — also on any installed workflow via **Check Required Nodes**
- The Workflows page groups your library **by folder**, with a folder tree to jump between them
- Model Browser cards show **Installed** / **Update Available** for imported workflows, and the details page can delete one again
- New **Edit Metadata** on workflow cards — rename, describe, and give any workflow a picture, including files you dropped in yourself years ago
#### New Feature: 🎨 Qwen Image & Krea 2 in Inference
- New workflow profiles with auto-detection, automatic text encoder + VAE pairing, and one-click recommended sampler settings (both need an up-to-date ComfyUI, so update your package if a workflow is rejected):
  - **Qwen Image** (2512) — the top-ranked open text-to-image model, with best-in-class text rendering
  - **Krea 2 Turbo** (8-step) and **Krea 2 Raw** — Krea's open-weights model, free for personal use under the Krea AI Community License
- Qwen-tagged CivitAI checkpoints now route to the DiffusionModels folder like Wan/Flux/Krea already did
#### New Feature: ⬇️ One-Click Text Encoder & VAE Downloads
- When your model needs a text encoder or VAE you don't have, the Model card says what's missing and offers the official download in one click — Flux, Flux.2 (4B/9B-aware), Z-Image, Anima, HiDream, Qwen Image, and Krea 2
- Only official shared components are offered, so any CivitAI fine-tune still works; progress shows in the card and finished files select themselves
#### New Feature: 🧩 Persistent Inference Layouts
- Rearranged panes in Inference now stick, one of our most-requested features on Discord, GitHub ([#1340](https://github.com/LykosAI/StabilityMatrix/issues/1340)), and our feature tracker ([1](https://lykos.ai/feature/8f55f8d5-c25a-437f-a2ba-4093e3984754), [2](https://lykos.ai/feature/c407cc98-e355-4e8d-98a5-23be854ed6c0)) alike — arrangements are kept per tab type across restarts, **saving a project** stores its layout in the `.smproj`, and **Restore Default Layout** resets the current tab in place
- Added **uv cache** management under **Settings → Package Environment** ([feature request](https://lykos.ai/feature/4dc8ab9d-e35f-436b-9a3f-d3abc2db1800)) — see the cache size and clear it with progress in **Activity**, plus an optional size limit and auto-cleanup toggle
- Downloads in the **Activity** popout now show downloaded and total size (e.g. 1.2 GB / 6.5 GB) next to the percentage and speed ([feature request](https://lykos.ai/feature/f571997c-3607-48db-a62f-b315fc733280))
- Model downloads now skip files you already have ([feature request](https://lykos.ai/feature/3dc89d55-4859-4484-bde9-5951ad87a48a)) — a hash match points you at the existing file instead of downloading a duplicate
- Added a **Quick-Hide Hotkey** for shared computers ([feature request](https://lykos.ai/feature/3dbfa0e1-650a-426a-906d-b81d6f414104)) — instantly minimize, or exit with no confirmation prompts. Off by default; bind it in **Settings → System**
- Added **Sort** options to the Inference model picker — file name, title, base model, type, size, and dates, remembered between sessions; search relevance still wins while you type
### Changed
...
- Removed the **OpenArt workflow browser**: OpenArt has discontinued its ComfyUI workflow community. Workflows you already imported stay fully usable in your library
- Updated **AI-Toolkit** installs to PyTorch 2.13.0 on CUDA 13.0 with Python 3.12, matching current upstream requirements ([#1714](https://github.com/LykosAI/StabilityMatrix/issues/1714)) — with a console warning at launch if your NVIDIA driver predates the 580 series
...
- Refreshed the model card badges across the **Checkpoint Manager** and the **CivitAI / CivArchive browsers** — compact chips instead of chunky pills, an update badge showing the release date, a distinct style for early-access-only updates, and clearer Installed / NSFW / Deleted states
- **Checkpoint Manager** search is now typo-tolerant, sharing the Inference picker's fuzzy matching (`#tag` search unchanged)
- **Triton & SageAttention** installs now pick up the latest wheels again — an upstream naming change had pinned them to older builds; the newest wheels fix black or noisy outputs on some setups
### Fixed
- Fixed custom node installs failing on legacy source builds that import `pkg_resources` (e.g. `cupy-wheel`) — extension install scripts now get the same `setuptools<82` constraint package installs already had
- Fixed Qwen Image Edit's model check accepting an incompatible Krea 2 text encoder; the correct 7B download is offered instead
- Fixed cloud-download entries in model dropdowns doing nothing until you clicked another item first, and the jumpy scrolling around them
- Fixed switching model families after a restart keeping the previous family's encoders and slot count
- Fixed companion downloads sometimes not filling their slot when the batch finished
- Fixed the Model Browser failing with "CivitAI can't be reached right now (OK: OK)" — CivitAI started returning `null` for some model statistics; missing stats now read as 0
- Fixed [#1695](https://github.com/LykosAI/StabilityMatrix/issues/1695) - models whose CivitAI page has since been deleted showing an error dialog on every click, with no way to break the link:
  - The details page falls back to your cached info, and **Next/Previous** skip deleted models instead of getting stuck on an error
  - A new right-click **Disconnect from Source** severs the dead link while keeping local metadata (name, description, thumbnail, trigger words)
- Fixed [#1708](https://github.com/LykosAI/StabilityMatrix/issues/1708) - OneTrainer on **Windows ROCm** crashing at the start of training with bitsandbytes 8-bit optimizers — the incompatible build is no longer installed, with a launch notice explaining why - thanks to @NeuralFault for the diagnosis!
- Fixed [#1710](https://github.com/LykosAI/StabilityMatrix/issues/1710) - CivitAI downloads sometimes fetching a different file from the same model version (then failing hash verification) when several files share identical metadata
...
- Fixed [#1715](https://github.com/LykosAI/StabilityMatrix/issues/1715) - the app crashing on startup on Linux when the models folder holds names differing only in case (e.g. `unet` and `Unet`, also [#1149](https://github.com/LykosAI/StabilityMatrix/issues/1149)/[#1357](https://github.com/LykosAI/StabilityMatrix/issues/1357)) — those folders now show up in the Checkpoint Manager instead of being silently skipped
- Fixed [#1707](https://github.com/LykosAI/StabilityMatrix/issues/1707) - Inference's **Layer Diffuse** addon failing at the sampler with `'NoneType' object is not subscriptable` on current ComfyUI — Inference offers the Inference Core Nodes update that carries the fix
- Fixed [#1717](https://github.com/LykosAI/StabilityMatrix/issues/1717) - Inference and Image Lab asking you to install a ComfyUI extension you already have (most often **ComfyUI-GGUF**), with the prompt returning after every install — registry installs, remotes differing only by a `.git` suffix or SSH form, and Windows-style git config line endings are all recognized now
- Fixed [#1716](https://github.com/LykosAI/StabilityMatrix/issues/1716) - phantom **Update Available** badges on embeddings, VAEs, upscalers, and ControlNets; wrong badges clear on the next check
- Fixed early access versions no longer being detected, breaking the early-access badge and the **Hide Early Access Models** filter
- Fixed multi-architecture models (e.g. Illustrious | Pony | NoobAI in one listing) counting a release for a different architecture as an update — checks now compare within your installed version's base model
- Fixed metadata scans always reporting success — the completion message now shows real counts, tells rate limiting apart from "not on CivitAI", and stops early instead of grinding your library into a rate limit
- Fixed **Update Metadata** skipping models imported from HuggingFace or OpenModelDB (now hashed on demand), and scans freezing the app on large files
- Fixed **Organize Models** appearing to do nothing out of the box — the default pattern is now `{base_model}/{file_name}` so the first preview proposes real folders (saved patterns untouched)
- Fixed model card text being hard to read in light theme
- Fixed the app sometimes crashing on exit with an **Inference** tab open, most often with **Remove folder links on shutdown** enabled
- Fixed a crash dialog appearing after you'd already closed the app — shutdown errors are now logged and the app just exits
- Fixed installing a package that needs a newer Python breaking an already-installed package's environment — a 3.13 package alongside an existing 3.12 one left the older one unable to launch with `error no: 2`, and hand-repairs were overwritten on every launch - thanks to @NeuralFault!
- Fixed **Clear Finished** in the **Activity** popout also clearing failed downloads, taking their **Retry** button with them — failed items now stay until you retry or dismiss them - thanks to @NeuralFault!
- Fixed **SwarmUI**'s **Launch Mode** option having no effect, as it used a hyphen where SwarmUI expects `--launch_mode` — the dropdown also gains a **none** option to skip the browser launch - thanks to @NeuralFault!
- Fixed **Forge Neo** and other uv-dependent packages failing to launch on **Linux** and **macOS** with `Error: 2  uv not found` ([#1713](https://github.com/LykosAI/StabilityMatrix/issues/1713)) — the bundled uv is now on the launch PATH there too - thanks to @NeuralFault!
### Supporters
#### 🌟 Visionaries
Three new model profiles, one-click downloads for the pieces that go with them, workflows you can browse and import from CivitAI, and Inference layouts that finally stay where you put them: this build is mostly people asking for things and us getting to say yes. Our Visionaries are why that's possible at all, so thank you **Waterclouds**, **MrMxyzptlk12836**, **bluepopsicle**, **Ibixat**, **Droolguy**, **snotty**, **dispenser**, **cusalapapen1481**, **moon_milky2843**, **CC**, and **SnooSnooEternal**. Every one of these features has a bit of your backing in it. And a warm hello to **TwistedDragon**, **Eaglebane**, and **hasezou11013179**, who join the Visionaries this time around; we're really glad you're here. 💛
...
>>>>>>> 3bd7afed (Merge pull request #1373 from ionite34/fix-civarchive-redesign-compat)
## v2.16.3
### Added
#### New Feature: 🔀 CivitAI Workflow Browsing
...
- Fixed installing a package that needs a newer Python breaking an already-installed package's environment — adding a package on Python 3.13 next to an existing 3.12 one left the older package unable to launch with `error no: 2`, and repairing it by hand didn't hold because it was rewritten on every launch - thanks to @NeuralFault!
- Fixed **Clear Finished** in the **Activity** popout also clearing failed downloads, taking their **Retry** button with them — failed items now stay until you retry or dismiss them yourself - thanks to @NeuralFault!
- Fixed **SwarmUI**'s **Launch Mode** option having no effect, as it used a hyphen where SwarmUI expects `--launch_mode` — the dropdown also gains a **none** option to stop SwarmUI opening a browser window on launch - thanks to @NeuralFault!
- Fixed **Forge Neo** and other uv-dependent packages failing to launch on **Linux** and **macOS** with `Error: 2  uv not found` ([#1713](https://github.com/LykosAI/StabilityMatrix/issues/1713)) — the bundled uv is now on the launch PATH there too, not just on Windows - thanks to @NeuralFault!
- Fixed the **CivArchive browser** returning far fewer results than the CivArchive website — the site's redesign quietly pinned the app's search route to the past quarter, and the renamed result-count field capped infinite scroll at a single page
- Fixed clicking some CivArchive results showing an error instead of the model — the redesign added whole-model search results whose detail pages load through a redirect the app didn't follow
- Fixed CivArchive file results opening in the web browser instead of the in-app details page, and the details page's version dropdown doing nothing — both casualties of a renamed field in the redesigned API
- CivArchive cards without a preview (whole-model and file results) now backfill an image from the model's gallery in the background instead of staying gray placeholders
...
- Added a **Bitsandbytes NF4** launch option to Stable Diffusion WebUI Forge - Neo for low-bit (`--bnb`) inference
- Added an **Activity center**: the sidebar download panel now has a **Notifications** tab alongside **In Progress**. Toasts are clickable — jumping to the downloaded folder, the originating page (e.g. Inference), or the activity panel — and persist into a session notification history (every notification is recorded, even ones suppressed by your settings) with read/unread indicators and a combined unread + active-download badge on the sidebar item
- Added **Gemini 3.1 Flash (Nano Banana 2)** as a new cloud provider in Image Lab — Google's latest fast image model, sitting between Gemini 2.5 Flash and Gemini 3 Pro in the dropdown. Uses the newer `thinking_level` config when thinking is enabled, and falls back to the model's default behavior otherwise
...
- Added **Steps and CFG sliders** to the Flux.2 Klein settings panel. The defaults snap automatically to the recommended values for the selected variant (4 / 1 for distilled, 20 / 5 for base — including community 9B fine-tunes and "base & distilled" merge listings), and you can override either at any time
- Added an **"Always Show Scrollbars"** toggle under **Settings → Appearance**. Defaults on — vertical scrollbars stay visible at their full thickness and reserve real layout space instead of fading to a thin overlay-style bar that only thickens on hover. Toggle off to restore Avalonia's classic auto-hide behavior. Single-line numeric inputs (e.g. SamplerCard Width/Height) keep their auto-hide regardless so spin-buttons aren't followed by a phantom bar
- Added new shared model folder categories — **Style Models**, **Audio Encoders**, **Model Patches**, and **Background Removal** — for ComfyUI's `style_models`, `audio_encoders`, `model_patches`, and `background_removal` directories. Models in these folders are now indexed and symlinked alongside everything else (e.g. Flux Redux / B-Lora style models, audio encoders for video/audio workflows, BiRefNet background-removal models)
- Added a **Download progress indicator** to the Image Lab status banner. While a model-download batch is in flight, the banner shows "⬇️ Downloading models (N/Total)..." with the count bumping as each file completes, instead of continuing to display the "missing models" warning
- Greatly expanded native **Windows ROCm (AMD GPU)** support ([#1629](https://github.com/LykosAI/StabilityMatrix/pull/1629)) — the GPU detection matrix now spans **Vega / GCN5** (Vega 56/64, Radeon VII) through the entire **RDNA1/2/3/3.5** lineup and into **RDNA4** (RX 9070 / R9700), using TheRock ROCm Technical Preview PyTorch builds. ROCm install and launch now run through a shared helper, so the same Windows-native path is available to **ComfyUI, SwarmUI** (its Comfy backend), **reForge, InvokeAI, and Wan2GP** - thanks to @NeuralFault!
- Added optional **ROCm package commands** for ComfyUI on Windows — one-click install of **SageAttention**, **Flash Attention**, **bitsandbytes**, and the **ROCm SDK devel** module (for compiling extensions/modules against your installed ROCm) from the package's command menu - thanks to @NeuralFault!
### Changed
- Tidied up the Inference SamplerCard dimensions section — Source/Presets actions are shown as labeled buttons below the dimension row
- Promoted the Encoder Type selector in the Inference Model card out of Advanced Options up to the main card body, so it's visible whenever a non-Auto workflow profile is active (and always when **Custom** is selected)
- Local model autocomplete in the prompt editor now uses substring matching instead of prefix-only — typing any part of a model's filename surfaces it, with names that start with your search still ranked first
- Updated the bundled Qwen Image Edit model used by Image Lab to the **2511** build (Alibaba's November 2025 refresh) for better edit consistency, reduced drift on multi-turn edits, and stronger character/geometry preservation. Existing 2509 downloads continue to work — only newly installed setups pull the new file
- Improved the Gemini API error message in Image Lab when the API returns 401/403 to point users at Google's API key restriction policy (which starts blocking unrestricted keys on June 19 2026)
- Updated AI-Toolkit to install torch 2.9.1 / torchvision 0.24.1 / torchaudio 2.9.1 from the cu128 index to match upstream (ostris/ai-toolkit), with a cu126 fallback for legacy NVIDIA GPUs; also pin numpy to 1.26.4 to avoid a numpy 2.x ABI break in scipy/diffusers that crashed training runs
- Pinned kohya_ss torch to 2.7.0 / torchvision 0.22.0 (cu128) to match upstream's requirements_pytorch_windows.txt instead of resolving an untested latest, keeping the cu126 legacy-GPU fallback
- Pinned reForge torch to 2.9.0 to match upstream (modules/launch_utils.py)
...
- The Inference checkpoint dropdown no longer **resets its scroll position** every time the model list refreshes. The refresh now applies a single combined (local + remote) diff to the underlying source cache, rather than first resetting to local-only and then re-adding remote entries — which previously caused the open dropdown to scroll back to the top
- **PyTorch TunableOp** is now disabled by default on Windows ROCm. If you have an existing TunableOp cache and want to keep tuning GEMM kernels, add `PYTORCH_TUNABLEOP_ENABLED=1` under **Settings → Environment Variables** - thanks to @NeuralFault!
### Fixed
- Fixed Inference text encoder selections being cleared when navigating away from and back to the Inference tab — encoder slots now ignore the transient null the model dropdown reports while its list refreshes
- Fixed [#1585](https://github.com/LykosAI/StabilityMatrix/issues/1585) - FluxGym installs/updates pulling an incompatible `transformers` version — installs now pin `transformers==4.54.1` and exclude it from the default requirements pass
- Fixed [#1641](https://github.com/LykosAI/StabilityMatrix/issues/1641) - Cogstudio failing to set up its `inference/gradio_composite_demo` directory when the parent path didn't already exist
...
- Fixed clicking a CivitAI model card with an empty version list appearing to do nothing for ~1–2s while the recovery round-trip runs — the clicked card now shows a "Loading..." state during the recovery, and the recovered version data is cached on the card so subsequent clicks are instant
- Fixed "Invalid download link" error when using the browser extension
- Fixed the Image Lab status banner **flickering** between "Click Connect" and the button+text variants every second while ComfyUI was starting up — the per-attempt retry loop was toggling `IsConnecting` on/off, which propagated through `CanUserConnect` to the button's visibility. The retry loop now holds `IsWaitingForConnection = true` for its entire duration so the banner stays parked on "🔄 Connecting to ComfyUI..."
...
- Fixed ComfyUI workflow rejections in Image Lab being surfaced as just "400 Bad Request" with no detail — the provider now logs the full JSON `node_errors` payload returned by ComfyUI so the failing node and validation error are visible in the log and the user-facing error message
...
- Fixed user-set environment variables not overriding package-configured ROCm variables on Windows — launch env vars now layer as **helper defaults → package config → user-set**, so anything you set under **Settings → Environment Variables** always wins - thanks to @NeuralFault!
