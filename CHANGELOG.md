...
- New workflow profiles with auto-detection, automatic text encoder + VAE pairing, and one-click recommended sampler settings:
  ...
  - Both need an up-to-date ComfyUI — update your package if the workflow is rejected
- Qwen-tagged CivitAI checkpoint files now route to the DiffusionModels folder like Wan/Flux/Krea already did
#### New Feature: ⬇️ One-Click Text Encoder & VAE Downloads
- When your selected model needs a text encoder or VAE you don't have, the Model card says exactly what's missing and offers the official downloads in one click — works for Flux, Flux.2 (4B/9B-aware), Z-Image, Anima, HiDream, Qwen Image, and Krea 2
- Your model choice is never touched — bring any CivitAI fine-tune; only the shared official components are offered, download progress shows right in the card, and the finished files select themselves when done
- The text-encoder dropdowns also list the official encoders as cloud-download entries, with installed files now sorted above downloadable ones
...
- Added **uv cache** management under **Settings → Package Environment** ([feature request](https://lykos.ai/feature/4dc8ab9d-e35f-436b-9a3f-d3abc2db1800)) — check the cache's size and clear it (after a quick explanation of what that means), with progress in the **Activity** panel and a report of the space freed. Optionally set a size limit to get a startup warning when the cache grows past it — off by default so launch stays lean — plus an automatic cleanup toggle
- Downloads in the **Activity** popout now show downloaded and total size (e.g. 1.2 GB / 6.5 GB) alongside the percentage and speed ([feature request](https://lykos.ai/feature/f571997c-3607-48db-a62f-b315fc733280))
- Model downloads now skip files you already have ([feature request](https://lykos.ai/feature/3dc89d55-4859-4484-bde9-5951ad87a48a)) — when a download's hash matches an installed model, a notification points at the existing file instead of downloading a duplicate
- Added a **Quick-Hide Hotkey** for shared computers ([feature request](https://lykos.ai/feature/3dbfa0e1-650a-426a-906d-b81d6f414104)) — press a shortcut to instantly minimize Stability Matrix, or exit with no confirmation prompts. Off by default; enable it in **Settings → System** and bind any combination with the game-style key picker (default Ctrl+Shift+H)
### Changed
...
### Fixed
- Fixed Qwen Image Edit's model check accepting an incompatible Krea 2 text encoder — the correct 7B encoder download is now offered instead
- Fixed clicking a cloud-download entry in model dropdowns sometimes doing nothing until you clicked a different item first
- Fixed jumpy, hard-to-reach scrolling near the cloud-download entries in model dropdowns
- Fixed switching model families after an app restart keeping the previous family's text encoders and slot count (e.g. Flux.2 Klein holding onto clip_l) — restored auto-picked components are now recognized as replaceable again; your own manual encoder picks stay protected
- Fixed companion downloads sometimes not filling their slot when the batch finished (e.g. the text encoder staying empty while the VAE filled) — each component now fills in as soon as its download completes, and the final pass waits for the model list to catch up
...
>>>>>>> bc144b3d (Merge pull request #1330 from ionite34/fix/trainer-install-updates)
## v2.16.2
