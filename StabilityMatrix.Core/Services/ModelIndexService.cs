...
public partial class ModelIndexService : IModelIndexService
{
    ...
    private async Task RefreshIndexParallelCoreV2()
    {
        if (!settingsManager.IsLibraryDirSet)
        {
            logger.LogTrace("Model index refresh skipped, library directory not set");
            ...
            .ForEachAsync(
                ...
                async (path, ct) =>
                {
                    ...
                    if (paths.Contains(jsonPath))
                    {
                        ...
                        {
                            // await using var stream = jsonPath.Info.OpenRead();
                            var text = await jsonPath.ReadAllTextAsync(ct).ConfigureAwait(false);

                            /*var connectedModelInfo = await JsonSerializer
                                .DeserializeAsync(
                                    stream,
                                    ConnectedModelInfoSerializerContext.Default.ConnectedModelInfo, ct).ConfigureAwait(false);*/
